...
func Test_reflectValueSetColumn(t *testing.T) {
	testCases := []struct {
		name    string
		cs      map[string][]byte
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			val := NewReflectValue(tc.val, meta) //  创建基于反射实现的Value对象
			cols := make([]string, 0, len(tc.cs))
			colVales := make([]driver.Value, 0, len(tc.cs))
			for k, v := range tc.cs {
				cols = append(cols, k)         // 遍历列
				colVales = append(colVales, v) // 遍历列值
			}
			// mock 模拟返回
			mock.ExpectQuery("SELECT *").WillReturnRows(sqlmock.NewRows(cols).AddRow(colVales...))
			...
			if tc.wantErr != nil {
				t.Fatal(fmt.Sprintf("Expected an error, but did not receive it:%+v", tc.wantErr))
			}
			assert.Equal(t, tc.wantVal, tc.wantVal)
		})
	}
}
